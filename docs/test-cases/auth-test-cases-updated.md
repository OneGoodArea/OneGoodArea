...
|---|---|---|---|---|
| **REG-01** | Sign-up page renders correctly | 1. Navigate to `/sign-up` | Two-column layout: left = brand panel (dark), right = form (cream). Title: "Start your free Sandbox." CTA: "Create account". |OK
...
| **REG-08** | Email already registered (credentials) | 1. Register with an email that already exists<br>2. Observe error | Error: "An account with this email already exists. Try signing in instead." (error code: `email_taken`, status 409). |OK
| **REG-09** | Email linked to Google account | 1. Register with an email already linked via Google OAuth<br>2. Observe error | Error: "This email is linked to a Google account. Try signing in with Google instead." (error code: `email_oauth`, status 409). |OK
...
| **REG-11** | Empty email submission | 1. Click "Create account" with empty email field | Form validation prevents submission via `required` attribute; API also returns 400 "Email is required" if bypassed. |OK(something went wrong)
| **REG-12** | Empty password submission | 1. Enter email only, leave password blank<br>2. Click "Create account" | Form validation prevents submission via `required` attribute; API returns 400 "Password must be at least 8 characters" if bypassed. |OK(something went wrong)
| **REG-13** | Password too short (<8 chars) | 1. Enter email + password of 7 characters<br>2. Click "Create account" | Client-side `minLength={8}` prevents submission; API returns 400 "Password must be at least 8 characters" if bypassed. |OK(something went wrong)
| **REG-14** | Password exactly 8 characters | 1. Enter email + password of exactly 8 characters<br>2. Click "Create account" | Registration succeeds. No client or server rejection. |NOK (NEW add option to view typed password and password checker)
| **REG-15** | Invalid email format | 1. Enter "not-an-email" as email<br>2. Click "Create account" | Browser-native `type="email"` validation prevents submission. |NOK. Account was created with email email<br>2, and a verification was supposed to be sent
...
| ID | Test Case | Steps | Expected Result | Status |
|---|---|---|---|---|
| **OAUTH-01** | Sign-up page shows OAuth buttons | 1. Navigate to `/sign-up`<br>2. Observe OAuth section | Two buttons above "or with email" divider: "Continue with Google" (colored G icon), "Continue with GitHub" (GitHub icon). |OK
...
| ID | Test Case | Steps | Expected Result | Status |
|---|---|---|---|---|
| **SIGNIN-01** | Sign-in page renders correctly | 1. Navigate to `/sign-in` | Two-column layout. Title: "Sign in." Subtitle: "Pick up where you left off..." OAuth buttons, "or with email" divider, email + password fields, "Forgot?" link, "Sign in" button, "New here? Create an account" footer. |OK
| **SIGNIN-02** | Successful sign-in with valid credentials | 1. Enter registered + verified email and correct password<br>2. Click "Sign in" | `signIn("credentials")` succeeds. Redirected to `callbackUrl` (default `/dashboard`). `router.refresh()` called. |OK
| **SIGNIN-03** | Invalid email (not registered) | 1. Enter an email that does not exist in the system<br>2. Enter any password<br>3. Click "Sign in" | Error: "Invalid email or password." User remains on `/sign-in`. Form is not cleared. |OK
| **SIGNIN-04** | Invalid password for registered email | 1. Enter correct email but wrong password<br>2. Click "Sign in" | Error: "Invalid email or password." User remains on `/sign-in`. Form is not cleared. |OK
| **SIGNIN-05** | Empty email field | 1. Leave email blank, enter any password<br>2. Click "Sign in" | Browser-native `required` validation prevents submission. |OK
| **SIGNIN-06** | Empty password field | 1. Enter email, leave password blank<br>2. Click "Sign in" | Browser-native `required` validation prevents submission. |OK
...
| **SIGNIN-11** | Sign-in with OAuth-only account via credentials | 1. User has a Google/GitHub-only account (no `password_hash`)<br>2. Attempt credentials sign-in with that email | `authorize()` returns `null`. Error: "Invalid email or password." |OK
...
| **SIGNIN-13** | "New here? Create an account" footer link | 1. On `/sign-in`<br>2. Click "Create an account" | Navigates to `/sign-up`. |OK
...
| ID | Test Case | Steps | Expected Result | Status |
|---|---|---|---|---|
| **SIGNIN-OAUTH-01** | Sign-in page shows OAuth buttons | 1. Navigate to `/sign-in`<br>2. Observe OAuth section | "Continue with Google" and "Continue with GitHub" buttons rendered above the divider. |OK 
| **SIGNIN-OAUTH-02** | Google sign-in — existing account | 1. Click "Continue with Google"<br>2. Authenticate with a previously-linked Google account | Sign-in succeeds. Redirected to `callbackUrl` (default `/dashboard`). |OK
...
| ID | Test Case | Steps | Expected Result | Status |
|---|---|---|---|---|
| **FW-01** | Forgot password page renders correctly | 1. Navigate to `/forgot-password` | Title: "Reset it in one email." Subtitle explains flow. Email field + "Send reset link" button. "← Back to sign in" link. |OK
| **FW-02** | Request password reset for existing account | 1. Enter registered email<br>2. Click "Send reset link" | POST `/api/auth/forgot-password`. Always returns `{ ok: true }`. UI transitions to success state: "Check your email." with 3-step checklist. |OK
...
| **FW-08** | Empty email submission | 1. Click "Send reset link" with empty field | Browser-native `required` validates. API returns 400 "Email is required" if bypassed. |OK
| **FW-09** | "Back to sign in" link works | 1. On `/forgot-password`<br>2. Click "← Back to sign in" | Navigates to `/sign-in`. |OK
| **FW-10** | "Back to sign in" on success screen | 1. After requesting reset<br>2. Click "Back to sign in" | Navigates to `/sign-in`. |OK
...
| **SESS-02** | Session persists across requests | 1. Sign in<br>2. Navigate to multiple pages (`/dashboard`, `/settings`, `/compare`) | User remains authenticated. No re-prompt for credentials. |OK
...
| ID | Test Case | Steps | Expected Result | Status |
|---|---|---|---|---|
| **SHELL-01** | Auth pages use two-column layout | 1. Navigate to any auth page (`/sign-in`, `/sign-up`, `/forgot-password`, `/reset-password`, `/verify`) | Left: brand panel (dark surface) with wordmark, tagline, and footer links. Right: form column (cream surface). |OK
| **SHELL-02** | Brand panel shows correct content | 1. Observe left panel on any auth page | Wordmark (links to `/`), eyebrow "OneGoodArea", headline "The data and intelligence layer underneath UK property workflows.", lead paragraph about 4 products + Sandbox free tier. Footer: "← Back to site", "Pricing", "About". |OK
| **SHELL-03** | "Back to site" link works | 1. Click "← Back to site" in brand panel footer | Navigates to `/`. |OK
| **SHELL-04** | "Pricing" link in brand panel | 1. Click "Pricing" in brand panel footer | Navigates to `/pricing`. |OK
| **SHELL-05** | "About" link in brand panel | 1. Click "About" in brand panel footer | Navigates to `/about`. |OK
...
| **EDGE-09** | Browser back button after sign-in | 1. Sign in<br>2. Press browser back button | Should not return to `/sign-in` with form data. Ideally redirects forward to `/dashboard`. |OK
| **EDGE-10** | Concurrent sign-in from multiple tabs | 1. Sign in in tab A<br>2. Switch to tab B (which shows `/sign-in`)<br>3. Observe | Tab B may still show stale state until refreshed. No conflict. |OK
