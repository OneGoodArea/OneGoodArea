...
| 2026-05-31 | **PR L — `/for/lenders` (second ICP page)** | Same skeleton as `/for/proptech`, Lender-specific content. Compliance / model-risk heavier than PropTech. **/for/lenders IA**: Hero (cream, "For lenders" eyebrow + "Versioned, pinnable area scoring your model risk team can defend." H1 + 2 CTAs) → § 01 The problem (cream-quiet, 3 paragraphs on model risk register + methodology versioning + continuous drift) → § 02 How it fits (DARK, 5 numbered steps: org-pin methodology → score at decision → save preset → monitor portfolio drift → audit-replayable AI) → § 03 Products you reach for (cream, 3 cards: Scores [primary] · Monitor · Intelligence, each w/ use case + real code + live link) → § 04 What you can defend (cream-quiet, **6** pillar cards w/ ADR cites — methodology pinning [0031], engine version body+header split [0008/AR-131], plan-replayable AI [0017], sample-size honest [0013/0014], **92.9% planner accuracy qualified w/ "14-case curated corpus" + Wilson CI** [0026], country-scoped percentiles [0005]) → § 05 FAQs (DARK, **7** lender-specific Q&As: pinning · semver · plan replay · latency · FCA/PRA SS1/23 · cross-env locking · what gets stored) → CTA (DARK, "Close the model-governance gap your auditors keep flagging"). **SEO**: unique title/meta/OG, 8-keyword array w/ lender-specific terms (model risk, FCA SS1/23, etc.), FAQPage JSON-LD w/ 7 questions loaded via next/script. **Slug normalisation**: /business `anchor: "lender"` → `"lenders"` to match `/for/lenders` + BuiltFor `icpSlug: "lenders"`. **Wiring**: READY_ICPS set gains "lenders"; Homepage Section 2 Lenders tab `ready: false` → `true`. **PR #89 (OyaAI spam) closed** at Pedro's request as part of housekeeping. | ✅ MERGED `bf8aae0` (PR #93) |
| 2026-05-31 | **PR M — `/for/insurance` (third ICP page)** | Same skeleton, Insurance + InsureTech-specific content. Monitor primary, then Scores configurable composite + Intelligence peer-relative anomaly. **/for/insurance IA**: Hero (cream, "For insurance and InsureTech" + "Area risk inputs the actuary can audit, plus continuous drift detection." + 2 CTAs) → § 01 The problem (cream-quiet, 3 paragraphs: black-box scores die in actuarial review · continuous drift mid-policy · peer-relative vs absolute risk lens) → § 02 How it fits (DARK, 5 numbered steps: save actuarial weighting as preset_id → score at quote time → save insured book as portfolio → detect drift continuously w/ signed webhooks → peer-relative anomaly via find_insights) → § 03 Products you reach for (cream, 3 cards: **Monitor (primary)** · Scores · Intelligence) → § 04 What you can defend (cream-quiet, **6** pillar cards: configurable weights + transparent components [0008/0030] · variance-aware confidence [v2.0.1 rubric] · sample-size gated change detection [0013/0014] · signed webhook delivery [0009/0013] · peer-relative not absolute [0023/0024] · methodology version pinning [0031]) → § 05 FAQs (DARK, **7** Q&As: actuarial weight tuning · rating-engine latency · webhook signing semantics · custom peer cohorts via Levers · renewal-cycle version locking · MGA / carrier-of-carriers fit · what gets stored) → CTA (DARK, "Configurable. Auditable. Continuously monitored. Sample-size honest."). **SEO**: 8-keyword array (InsureTech, MGA, actuarial area scoring, peer-relative anomaly, signed webhook risk alerts, etc.) + FAQPage JSON-LD w/ 7 questions. **Wiring**: READY_ICPS gains "insurance"; Homepage Section 2 Insurance tab `ready: true`. **Branch-protection milestone**: Pedro added himself to bypass list but rule still blocked; merged via `--admin` (bypass-list config to revisit). | ✅ MERGED `7e3611a` (PR #94) |
| 2026-05-31 | **PR N — `/for/cre` (fourth ICP page)** | Same skeleton, CRE + site-selection content. Intelligence primary (compound rank_areas via `/v1/query`), Signals second (single-signal `/v1/areas`), Scores third (business profile). **/for/cre IA**: Hero (cream, "For CRE and site selection" + "Screen the whole UK against your site criteria in one typed call." + 2 CTAs) → § 01 The problem (cream-quiet, 3 paragraphs on ranking-at-portfolio-scale + spreadsheet stitching + quarterly reproducibility) → § 02 How it fits (DARK, 5 steps: compound rank in one call → drill into shortlisted areas via /v1/area → find peers of best-performing catchment → score with business profile → replay next quarter) → § 03 Products (cream, 3 cards: **Intelligence (primary)** · Signals · Scores) → § 04 What you can defend (cream-quiet, **6** pillar cards w/ ADR cites — compound queries one trip [0019] · reproducible shortlists via plan echo [0017] · materialised peer graph [0023/0024] · ONS-backed scope no fake polygons [0005/0032] · deterministic + stable [0008/0017] · country-scoped percentiles [0005]) → § 05 FAQs (DARK, **7** Q&As: `/v1/areas` vs `/v1/query` semantics · screen scale 1000 rows per call · customising commercial dimensions · non-LSOA catchments via Levers peer cohorts · footfall data honesty [we do not ingest it] · how peer math actually computes · quarterly replay flow) → CTA (DARK, "One typed query against the whole UK. Reproducible next quarter."). **SEO**: 8-keyword array (CRE site selection API, catchment screening API, store location data API, etc.) + FAQPage JSON-LD w/ 7 questions. **Wiring**: READY_ICPS gains "cre"; Homepage Section 2 CRE tab `ready: true`. **4 of 5 ICP pages live; only Public sector remains.** | ✅ MERGED `7049965` (PR #95) |
| 2026-05-31 | **PR O — `/for/public-sector` (fifth + final ICP page)** | Last ICP page. Public sector + research (councils, central-gov analytical units, regeneration bodies, research institutes). Defensibility heavier than CRE; FOI + procurement-review survival is the whole pitch. **/for/public-sector IA**: Hero (cream, "For public sector and research" + "Sourced, dated, methodology-stamped UK area metrics that survive FOI." + 2 CTAs) → § 01 The problem (cream-quiet, 3 paragraphs: every number defends itself · cross-border lies [England's IMD vs Scotland's SIMD] · reproducibility across the contract cycle) → § 02 How it fits (DARK, 5 steps: resolve area w/ sourced signals → triage w/ research baseline profile → lock methodology for the contract cycle → track priority LSOAs as a portfolio → audit-replayable analyst queries) → § 03 Products (cream, **all 4 products** — only ICP page to use Signals + Scores + Monitor + Intelligence together) → § 04 What you can defend (cream-quiet, **6** pillar cards: country-scoped percentiles by design [0005] · provenance on every signal [0001/0002] · sample-size honest [0013/0014] · plan-replayable AI [0017] · methodology version pinning for the contract cycle [0031/AR-197] · published methodology + open ADR repo) → § 05 FAQs (DARK, **7** Q&As: FOI defence · pinning for procurement · **Northern Ireland honest gap [not yet covered]** · citing in research notes · statistical CI vs availability confidence · ward/MSOA aggregation roadmap · what's stored). CTA: "Sourced, dated, methodology-stamped. Built for the publication." **SEO**: 8-keyword array (council planning data API, FOI-survivable area metrics, MHCLG data, etc.) + FAQPage JSON-LD w/ 7 questions. **Wiring**: READY_ICPS gains "public-sector"; Homepage Section 2 Public sector tab `ready: true`. **🎉 All 5 ICP pages live. No more disabled "Soon" pills anywhere across the homepage Section 2 or /business ICP cards.** | ✅ Approved on localhost |
