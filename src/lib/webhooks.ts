...
import {
  ensureWebhookSubscriptionsTable,
...
/* AR-129: outbound webhook subscriptions + signed delivery.
   ...
   MVP delivery is fire-and-forget synchronous from the API route — failed
   deliveries get recorded in `webhook_deliveries` with status='failed' but
   don't auto-retry yet. Phase 2 cron drains `failed` rows with exponential
   backoff. */
...
async function ensureTables() {
  ...
  await ensureWebhookSubscriptionsTable();
