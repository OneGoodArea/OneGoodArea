...
The environment will consist of a multi-container setup using `docker-compose`:
...
- **Unified `docker-compose.yml`:** Optimized for both Docker (Windows) and Podman (Linux).
- **Troubleshooting Layer:** The `Dockerfile.dev` will include `curl`, `iputils-ping`, `net-tools`, and `vim`.
...
1. **`feat/local-env-infra`**:
   - Add `docker-compose.yml` and `Dockerfile.dev`.
...
## 6. Post-Constraint Roadmap: Future Enhancements

Once the local test environment is stable and the "No Existing Code Modification" constraint is relaxed, the following items should be implemented to improve system architecture and maintainability:

### A. Native Environment Integration
- **`instrumentation.ts` / `layout.tsx` Integration:** Replace the external wrapper script/`--env-file` approach by importing `src/lib/test-env-loader.ts` directly into Next.js entry points. This ensures the test environment is automatically initialized without special startup flags.
- **Unified CSP Management:** Refactor `next.config.ts` to dynamically generate CSP headers based on the environment (Development vs. Production), removing the need for hardcoded local URLs in the production-ready configuration.

### B. Architectural Refactoring for Mocks
- **Dependency Injection / Provider Pattern:** Instead of relying on environment variable branching (e.g., `RESEND_API_BASE_URL`) within the business logic, refactor the application to use a dedicated "Provider" pattern. This will allow the application to inject different service implementations (Mock vs. Real) at the infrastructure layer.
- **Centralized API Client Configuration:** Consolidate all third-party API configurations (Stripe, Anthropic, Postcodes.io) into a single module that automatically handles mock/real switching based on the environment.

### C. Database & Schema Alignment
- **Integrated Migrations:** Merge `tests/local-db-init.sql` into the main Drizzle/migration pipeline. This ensures that the local test database schema is always in sync with production without maintaining a separate initialization script.
- **Seed Data Lifecycle:** Implement a robust seeding mechanism within the main `npm` scripts that can populate the local database with consistent test data for different scenarios (e.g., `npm run db:seed:standard`, `npm run db:seed:edge-cases`).

### D. Testing Utility Consolidation
- **Auth Testing Utilities:** Move the logic from `src/app/api/auth/auto-login/route.ts` into a standard testing utility library. This would allow integration tests to programmatically handle authentication without needing a dedicated HTTP endpoint.
- **Global Mock Cleanup:** Remove any "bypass" logic or temporary routes once the E2E testing framework (e.g., Playwright or Cypress) is fully integrated and can handle these operations via standard browser automation.
