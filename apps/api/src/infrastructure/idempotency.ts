...
import { rows } from "./db/types";
import { AppError } from "./errors/custom-errors";

/* AR-128: Stripe-style idempotency for the public REST API. Migrated from
   legacy src/lib/idempotency.ts. The ensureTable() self-create is dropped (the
   migrator owns idempotency_records). Wraps a cost-bearing handler so retries
   with the same `Idempotency-Key` header return the cached response instead of
