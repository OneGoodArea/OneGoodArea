...
export function buildPlannerPrompt(question: string): string {
  return [
    ...
    ``,
    `### compare_areas — side-by-side profiles for 2..5 areas`,
    `Use this WHENEVER the question names more than one area (postcode or`,
    `place). NEVER use get_area and drop the other postcodes — the consumer`,
    `expected a comparison and silently picking one is a bug.`,
    `{"op":"compare_areas","params":{`,
    `  "areas": ["<postcode or place>", ...]   (2..5 entries, in the order the user mentioned them)`,
    `}}`,
    ``,
    ...
    `}}`,
    ``,
    `### find_forecast — linear time-series projection for one signal at one LSOA`,
    `{"op":"find_forecast","params":{`,
    `  "target": {"geo_code":"<LSOA code>"} | {"postcode":"<postcode>"} | {"area":"<place name>"},   (EXACTLY one)`,
    `  "signal_key": "<any signal that has a monthly time-series, e.g. property.median_price or crime.monthly_count>",`,
    `  "window_months": 6..120   (optional, default 24 — how much history to fit),`,
    `  "horizon_months": 1..60   (optional, default 12 — how many months to project ahead)`,
    `}}`,
    ``,
    `## Hard rules`,
    ...
    ``,
    `Q: "compare M1 1AE and EC1A 1BB"`,
    `A: {"op":"compare_areas","params":{"areas":["M1 1AE","EC1A 1BB"]}}`,
    ``,
    `Q: "M20 2RN or LS6 4DP for a family?"`,
    `A: {"op":"compare_areas","params":{"areas":["M20 2RN","LS6 4DP"]}}`,
    ``,
    ...
    ``,
    `Q: "forecast the next 12 months of median house price in M1 1AE"`,
    `A: {"op":"find_forecast","params":{"target":{"postcode":"M1 1AE"},"signal_key":"property.median_price","horizon_months":12}}`,
    ``,
    `## The question`,
    question,
  ].join("\n");
