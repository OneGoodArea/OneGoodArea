...
/** End of the current UTC day (23:59:59.999). Used as the expiry for
    auto-provisioned playground keys (AR-595, Plan 059.3) — a fresh
    signed-in visitor gets a full session, but the key doesn't outlive
...
/** AR-595 (Plan 059.3): auto-provision a key for a logged-in playground
    visitor who has none of their own — only called when
    `provisionPlaygroundApiKey` has already established that. `auto_generated
    = true` and an end-of-day expiry distinguish it from a deliberately
    created key; AR-596 (Plan 059.4) rejects it on sensitive routes
    regardless of expiry. */
export async function createAutoGeneratedApiKey(
  userId: string,
  name: string = "Playground (auto-generated)",
): Promise<{ id: string; key: string; name: string; expiresAt: Date }> {
  const id = generateId("key");
  const key = `oga_${crypto.randomBytes(24).toString("hex")}`;
  const hash = hashApiKey(key);
  const preview = apiKeyPreview(key);
  const expiresAt = endOfTodayUtc();

  await repo.insertAutoGenerated(id, hash, preview, userId, name, expiresAt);

  return { id, key, name, expiresAt };
}

/** AR-595 (Plan 059.3): the orchestration the playground's "get me a
    working key" call needs. If the user already has ANY usable key
    (real or a still-valid auto-generated one), returns null — its
    plaintext can never be retrieved (only the hash is stored), so the
    caller falls back to telling the user to use their own key. Otherwise
    creates a fresh auto-generated, end-of-day key and returns it. */
export async function provisionPlaygroundApiKey(
  userId: string,
): Promise<{ key: string; expiresAt: Date } | null> {
  if (await repo.hasActiveKey(userId)) return null;
  const created = await createAutoGeneratedApiKey(userId);
  return { key: created.key, expiresAt: created.expiresAt };
}
...
export interface ValidatedApiKey {
  ...
  /** AR-595 (Plan 059.3) — true for a key the system auto-provisioned for
      a logged-in playground visitor. AR-596 (Plan 059.4) uses this to
      reject these keys on sensitive routes. Optional so existing test
      mocks keep type-checking; treat `undefined` as `false`. */
...
export async function validateApiKey(key: string, requestIp?: string | null): Promise<ValidateApiKeyResult> {
  ...
  // AR-595 (Plan 059.3): a past expires_at is treated exactly like
  // revoked=TRUE — the repo query already excludes revoked rows, this
  // covers the other way a key stops being valid. NULL never expires.
  ...
  const autoGenerated = found.auto_generated ?? false;
  ...
  return { userId, orgId, allowedIpCidrs, trainingOptout, autoGenerated };
}
