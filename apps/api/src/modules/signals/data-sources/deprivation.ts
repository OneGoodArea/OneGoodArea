...
/* Migrated VERBATIM from legacy src/lib/data-sources/deprivation.ts. Changes:
   DeprivationData imported from ../inputs (canonical) instead of re-declared,
   and each ArcGIS res.json() is cast to a minimal boundary type (Node's
   undici types it as unknown vs any in the Next/DOM lib). Runtime unchanged. */

/* ── ArcGIS response shapes (only the fields read) ── */
