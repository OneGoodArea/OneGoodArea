...
import type { OfstedSchool, OfstedData } from "../inputs";

/* Migrated from legacy src/lib/data-sources/ofsted.ts. Changes: sql repointed
   to infrastructure/db/client, logger to modules/tracking, OfstedSchool +
   OfstedData imported from ../inputs (canonical). The legacy ensureOfstedTable()
   self-create is dropped: the migrator (infrastructure/db/schema.ts) now owns
   the byte-identical ofsted_schools DDL + indexes, so modules issue no DDL.
   Query, haversine filter and prompt formatting are otherwise verbatim. */
