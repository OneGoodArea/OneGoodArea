 ...
 * Uses PBKDF2-SHA256 via the Web Crypto API (available in Node 20+ as
 * globalThis.crypto, same as the Edge runtime the legacy module targeted).
 *
 ...
 *
 * Migrated VERBATIM from legacy src/lib/crypto.ts (pure Web Crypto, no imports).
 */
...
// salt is constrained to an ArrayBuffer-backed view (not ArrayBufferLike, which
// would include SharedArrayBuffer) so it satisfies deriveBits' BufferSource
// param under the Node lib without a DOM-specific cast. Both callers produce
// exactly this via `new Uint8Array(n)`.
async function derivePBKDF2(password: string, salt: Uint8Array<ArrayBuffer>): Promise<ArrayBuffer> {
  ...
  return globalThis.crypto.subtle.deriveBits(
    {
      name: "PBKDF2",
      // salt is a Uint8Array (a valid BufferSource at runtime); the legacy
      // `as BufferSource` cast was DOM-lib specific and isn't needed under the
      // Node lib apps/api compiles against.
      salt,
