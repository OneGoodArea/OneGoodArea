...
import { effectiveEngineVersionForCaller, resolveBundleForCaller } from "../shared/bundles";
  ...
  const guardSignalsCtx = async (
    ...
    app.post("/v1/query",
      ...
      try {
        ...
        if (isAppError(error)) return reply.code(error.statusCode as 200 | 400 | 404 | 422 | 429 | 500).send({ error: error.message, code: error.code });
    ...
    app.post("/v1/peers",
      ...
      try {
        ...
        if (isAppError(error)) return reply.code(error.statusCode as 200 | 400 | 404 | 422 | 500).send({ error: error.message, code: error.code });
    ...
    app.post("/v1/insights",
      ...
      try {
        ...
        if (isAppError(error)) return reply.code(error.statusCode as 200 | 400 | 404 | 500).send({ error: error.message, code: error.code });
