...
describe("resolveEngineVersion", () => {
  ...
  describe("valid versions (supported window)", () => {
    ...
    it("trims whitespace around the version value", () => {
      const r = resolveEngineVersion("  1.0.0  ");
      expect(r.ok).toBe(true);
      if (r.ok) expect(r.requestedVersion).toBe("1.0.0");
    });
  });
  ...
  describe("unknown versions (not in the registry)", () => {
    it.each(["2.0.0", "1.1.0", "9.9.9"])(
      ...
      (version) => {
        ...
        if (!r.ok) {
          ...
          expect(r.supportedVersions).toContain("1.0.0");
        }
      },
    );
    ...
    it("rejects a leading-v prefix (strict matching, no normalisation)", () => {
      const r = resolveEngineVersion("v1.0.0");
      expect(r.ok).toBe(false);
    });
  ...
  it("getSupportedEngineVersions returns the supported window", () => {
    ...
    expect(supported).toContain("1.0.0");
  ...
  describe("orgPin option (Levers AR-197)", () => {
    it("uses the orgPin as the requestedVersion when no header is sent", () => {
      ...
      expect(r.ok).toBe(true);
      if (r.ok) {
        expect(r.requestedVersion).toBe("1.0.0");
        expect(r.resolvedVersion).toBe(METHODOLOGY_VERSION);
      }
    });

    it("uses the orgPin when header is undefined", () => {
      const r = resolveEngineVersion(undefined, { orgPin: "1.0.0" });
      expect(r.ok).toBe(true);
      if (r.ok) expect(r.requestedVersion).toBe("1.0.0");
    });

    it("honours an explicit valid header alongside a pin", () => {
      const r = resolveEngineVersion("1.0.0", { orgPin: "1.0.0" });
      expect(r.ok).toBe(true);
      if (r.ok) expect(r.requestedVersion).toBe("1.0.0");
    });
    ...
    it("falls through to METHODOLOGY_VERSION when orgPin is no longer in the supported window (defense in depth)", () => {
      ...
      // that's now unsupported, the read path silently falls back to
      // latest rather than 500ing every request. The pin row stays in
      // the DB for audit; a future commit can surface a deprecation event.
      const r = resolveEngineVersion(null, { orgPin: "2.0.0" });
      expect(r.ok).toBe(true);
      if (r.ok) expect(r.requestedVersion).toBe(METHODOLOGY_VERSION);
    });

    it("an EXPLICIT header still 400s on an unknown version even when orgPin is valid", () => {
      // Header path validation is unchanged; a valid pin never masks a
      // bad header.
      const r = resolveEngineVersion("2.0.0", { orgPin: "1.0.0" });
      expect(r.ok).toBe(false);
    });
  });
});
