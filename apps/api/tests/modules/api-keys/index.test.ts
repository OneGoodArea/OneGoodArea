...
import { hashApiKey, apiKeyPreview, createApiKey, listApiKeys, revokeApiKey, validateApiKey } from "@/modules/api-keys/index";
...
describe("validateApiKey", () => {
  it("returns the user id + org id + allowlist for a known hash", async () => {
    mockSql
      .mockResolvedValueOnce([{ user_id: "user_42", org_id: "org_user_42", allowed_ip_cidrs: [], training_optout: false }] as never) // SELECT
  ...
  it("still validates a legacy aiq_ key (no prefix gate, pure hash lookup); orgId null if backfill hasn't reached it", async () => {
    ...
    mockSql
      .mockResolvedValueOnce([{ user_id: "legacy_user", org_id: null, allowed_ip_cidrs: [], training_optout: false }] as never)
  ...
  it("returns blocked when the key has CIDRs and the request IP doesn't match", async () => {
    mockSql
      .mockResolvedValueOnce([{ user_id: "u1", org_id: "org_u1", allowed_ip_cidrs: ["10.0.0.0/8"], training_optout: false }] as never)
      .mockResolvedValue([] as never);
  ...
  it("empty allowlist = no restriction (any IP works, even missing)", async () => {
    mockSql
      .mockResolvedValueOnce([{ user_id: "u1", org_id: "org_u1", allowed_ip_cidrs: [], training_optout: false }] as never)
      .mockResolvedValue([] as never);
    expect(await validateApiKey("oga_x", "8.8.8.8")).toEqual({
      userId: "u1",
      orgId: "org_u1",
