...
import {
  ...
  revokeApiKey,
  validateApiKey,
  createAutoGeneratedApiKey,
  provisionPlaygroundApiKey,
...
describe("validateApiKey — expiry (AR-595, Plan 059.3)", () => {
  it("returns null for a key whose expires_at is in the past", async () => {
    mockSql
      .mockResolvedValueOnce([{
        user_id: "u1", org_id: "org_u1", allowed_ip_cidrs: [], training_optout: false,
        auto_generated: true, expires_at: new Date(Date.now() - 1000).toISOString(),
      }] as never)
      .mockResolvedValue([] as never);
    expect(await validateApiKey("oga_expired")).toBeNull();
  });

  it("still validates an auto-generated key before its expiry, surfacing autoGenerated: true", async () => {
    mockSql
      .mockResolvedValueOnce([{
        user_id: "u1", org_id: "org_u1", allowed_ip_cidrs: [], training_optout: false,
        auto_generated: true, expires_at: new Date(Date.now() + 60_000).toISOString(),
      }] as never)
      .mockResolvedValue([] as never);
    expect(await validateApiKey("oga_stillgood")).toEqual({
      userId: "u1",
      orgId: "org_u1",
      allowedIpCidrs: [],
      trainingOptout: false,
  ...
  it("never expires a key with expires_at = NULL", async () => {
    mockSql
      .mockResolvedValueOnce([{
        user_id: "u1", org_id: "org_u1", allowed_ip_cidrs: [], training_optout: false,
        auto_generated: false, expires_at: null,
      }] as never)
      .mockResolvedValue([] as never);
    expect(await validateApiKey("oga_permanent")).not.toBeNull();
  });
});

describe("createAutoGeneratedApiKey (AR-595, Plan 059.3)", () => {
  it("mints an oga_ key marked auto_generated with an end-of-day expiry", async () => {
    mockSql.mockResolvedValue([] as never);

    const { id, key, expiresAt } = await createAutoGeneratedApiKey("user_1");

    expect(id).toMatch(/^key_/);
    expect(key).toMatch(/^oga_[0-9a-f]{48}$/);
    ...
    expect(call[2]).toBe(hashApiKey(key));
    expect(call[4]).toBe("user_1");
    expect(call).not.toContain(key); // plaintext never bound as a param either
  });
});

describe("provisionPlaygroundApiKey (AR-595, Plan 059.3)", () => {
  it("returns null when the user already has an active key (plaintext isn't retrievable)", async () => {
    mockSql.mockResolvedValueOnce([{ x: 1 }] as never); // hasActiveKey finds a row
    expect(await provisionPlaygroundApiKey("user_1")).toBeNull();
  });

  it("creates and returns a fresh key when the user has none", async () => {
    mockSql
      .mockResolvedValueOnce([] as never) // hasActiveKey: none found
      .mockResolvedValue([] as never);    // the INSERT
    const result = await provisionPlaygroundApiKey("user_1");
    expect(result).not.toBeNull();
    expect(result?.key).toMatch(/^oga_[0-9a-f]{48}$/);
  });
});
