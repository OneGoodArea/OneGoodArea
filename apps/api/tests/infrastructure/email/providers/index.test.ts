...
describe("getEmailProvider", () => {
  it("selects MailHog when OGA_EMAIL_PROVIDER=mailhog", async () => {
    ...
    const { getEmailProvider } = await import("@/infrastructure/email/providers/index");
    const { MailhogEmailProvider } = await import("@/infrastructure/email/providers/mailhog-provider");
  ...
  it("defaults to Resend and constructs it with RESEND_API_KEY", async () => {
    ...
    const { getEmailProvider } = await import("@/infrastructure/email/providers/index");
    const { ResendEmailProvider } = await import("@/infrastructure/email/providers/resend-provider");
