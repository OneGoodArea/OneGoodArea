...
describe("GET /keys", () => {
  ...
  it("lists the caller's keys", async () => {
    mockList.mockResolvedValue([{ id: "key_1", key_preview: "oga_abcd", name: "Default", created_at: "2026-01-01T09:00:00.000Z", last_used_at: null, training_optout: false }] as never);
    const res = await app.inject({ method: "GET", url: "/keys", headers: AUTH });
    expect(res.statusCode).toBe(200);
    expect(res.json().keys).toEqual([{ id: "key_1", key_preview: "oga_abcd", name: "Default", created_at: "2026-01-01T09:00:00.000Z", last_used_at: null, training_optout: false }]);
  ...
  it("also accepts a real API key (bearerAuth), now that it's testable from Scalar (AR-596)", async () => {
    ...
    mockList.mockResolvedValue([{ id: "key_2", key_preview: "oga_efgh", name: "Default", created_at: "2026-01-01T09:00:00.000Z", last_used_at: null, training_optout: false }] as never);
