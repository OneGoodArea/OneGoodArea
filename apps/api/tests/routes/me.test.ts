...
describe("GET /me/activity", () => {
  it("401s when the Authorization header is missing", async () => {
    // AR-596 (Plan 059.4): this route now also accepts bearerAuth (a real
    // API key), so the outer requireCredential hook — which fires before
    // the handler and never calls session/key verification — uses the
    // API-key-flavoured message when no header is present at all.
    const res = await app.inject({ method: "GET", url: "/me/activity" });
    expect(res.statusCode).toBe(401);
    expect(res.json().error).toBe("Missing API key. Use: Authorization: Bearer oga_...");
    expect(mockSessionVerify).not.toHaveBeenCalled();
  });

  it("401s when the token is neither a valid session nor a valid API key", async () => {
    mockSessionVerify.mockResolvedValue(null);
    mockValidate.mockResolvedValue(null); // AR-596: OR-auth falls back to a real key check
  ...
  it("403s an auto-generated playground key (AR-595) even though it's a real, valid key", async () => {
    mockSessionVerify.mockResolvedValue(null);
    mockValidate.mockResolvedValue({ userId: "user_1", orgId: null, autoGenerated: true } as never);
    const res = await app.inject({
      method: "GET",
      url: "/me/activity",
      headers: { authorization: "Bearer oga_autogenerated" },
    });
  ...
  it("serialises a driver Date created_at to an ISO string (does not 500)", async () => {
    ...
    mockSql
      .mockResolvedValueOnce([
        { id: "evt_1", user_id: "user_1", event: "api.score.computed", metadata: {}, created_at: createdAt },
      ] as never)
      .mockResolvedValueOnce([{ total: 1 }] as never);
    ...
    expect(res.statusCode).toBe(200);
    expect(res.json().events[0].created_at).toBe(createdAt.toISOString());
  });
